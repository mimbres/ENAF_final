...
import numpy as np
import os
import EATS.generator_utils.cast.core as cast
import soundfile as sf

testdata_folder = '../fingerprint_dataset_dejavu/query/'
duration = 6  # in sec
music_folder = '../fingerprint_dataset_dejavu/music/'
noise_folder = '../fingerprint_dataset_dejavu/aug/'
ir_folder = '../fingerprint_dataset_dejavu/ir/'
n_test = int(6248 * 4)  # 0 denotes the number of all songs in music_folder
target_fs = 8000
...
### 2019.10.17
if n_test > len(filenames):
    filenames = filenames * int((n_test/len(filenames)) + 1)
###
cnt = 0
for filename in filenames: # music file
  ...
  x_bg, _ = librosa.core.load(os.path.join(noise_folder,noise_names[noise_idx]), sr=target_fs)
  ...
  x_ir, _ = librosa.core.load(os.path.join(ir_folder,ir_names[ir_idx]), sr=target_fs)
  ...
  for snr in snr_list:
    ...
    #librosa.output.write_wav(os.path.join(testdata_folder,str(snr),'{}_{}sec_{}noise_{}ir_{}dB.wav'.format(filename[:-4],front_margin_sec,noise_idx,ir_idx,snr)), x_aug, fs, subtype='PCM_16')
    output_file_path = os.path.join(testdata_folder,str(snr),'{}_{}sec_{}noise_{}ir_{}dB.wav'.format(filename[:-4],front_margin_sec,noise_idx,ir_idx,snr))
    sf.write(output_file_path, x_aug, fs, 'PCM_16')
