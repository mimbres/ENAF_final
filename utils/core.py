...
def log_scale_random_number(range=(0.001,1)):
  '''
    log_scale_random_number(range=(0.001,1))
      Function to pick a real-number in the input range.
      The output value is uniformly distributed in the log-scale axis.
      (i.e., The output value has the same probability to be between 0.01~0.1 and 0.1~1)

    arguments
      range: Range of the output. If not providedm the default range is used: [0.001,1).
             range[0] is the lower-bound of the output.
             range[1] is the upper-bound of the output.

    return
      A real-number that is uniform randomly picked in the input range when it is transformed into the log scale.
  '''

  range_log = np.log10(range)
  random_number_log = random_number(range_log)
  random_number_linear = np.power(10,random_number_log)

  return random_number_linear
