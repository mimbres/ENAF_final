...
CUDA_VISIBLE_DEVICES=0 python train_with_EATS_v2fix_batch_NTxent_simCLR_120win1_qsepLastFCv2.py exp_NTxent_simCLR_use_anc_rep_mod_120_qsepLastFCv2
CUDA_VISIBLE_DEVICES=0 python train_with_EATS_v2fix_batch_NTxent_simCLR_120win1_qsepLastFCv2.py exp_NTxent_simCLR_use_anc_rep_mod_120_qsepLastFCv2 '1' mini-test-only
CUDA_VISIBLE_DEVICES=1 python train_with_EATS_v2fix_batch_NTxent_simCLR_120win1_qsepLastFCv2.py exp_NTxent_simCLR_use_anc_rep_mod_120_qsepLastFCv2'19' save-db-query # 3:99.39
CUDA_VISIBLE_DEVICES=1 python eval.py logs/emb/exp_NTxent_simCLR_use_anc_rep_120/19 100
...
from model.nnfp_l2norm_v2_fixed_q_adapt_sepLastFCv2 import FingerPrinter
...
m_fp = FingerPrinter(emb_sz=EMB_SZ, fc_unit_dim=[32, 1], norm=BN, q_adapt_layer=USE_Q_ADAPT)
...
def train_step(Xa, Xp):
    ...
    with tf.GradientTape() as t:
        if USE_Q_ADAPT:# UPDATE for query-adaptive-layer 23. Apr. 2020.
            emb_key, emb_que = m_fp(feat)
            loss, sim_mtx, _ = loss_obj_tr(emb_org=emb_key[:TR_N_ANCHOR, :],
                                           emb_rep=emb_que[TR_N_ANCHOR:, :])
        else:
            emb = m_fp(feat)  # (B, E)
...
def val_step(Xa, Xp):
    ...
    if USE_Q_ADAPT:# UPDATE for query-adaptive-layer 23. Apr. 2020.
            emb_key, emb_que = m_fp(feat)
            loss, sim_mtx, _ = loss_obj_tr(emb_org=emb_key[:VAL_N_ANCHOR, :],
                                           emb_rep=emb_que[VAL_N_ANCHOR:, :])
    else:
        emb = m_fp(feat)  # (B, E)
        loss, sim_mtx, _ = loss_obj_tr(emb_org=emb[:VAL_N_ANCHOR, :],
...
def test_step(Xa, Xp, drop_div_enc=False):
    ...
    if drop_div_enc:
        ...
        if USE_Q_ADAPT:# UPDATE for query-adaptive-layer 23. Apr. 2020.
            emb_key, emb_que = m_fp(feat)
            return emb_key[:TS_N_ANCHOR, :], emb_que[TS_N_ANCHOR:, :]
        else:
            emb = m_fp(feat)  # (B, E) # Use g(f(x))                
