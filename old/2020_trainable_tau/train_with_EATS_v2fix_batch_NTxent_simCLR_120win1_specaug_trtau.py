...
CUDA_VISIBLE_DEVICES=0 python train_with_EATS_v2fix_batch_NTxent_simCLR_120win1_specaug_trtau.py exp_NTxent_simCLR_120_specaug_trtau
CUDA_VISIBLE_DEVICES=0 python train_with_EATS_v2fix_batch_NTxent_simCLR_120win1_specaug_trtau.py exp_NTxent_simCLR_120_specaug_trtau '1' mini-test-only
CUDA_VISIBLE_DEVICES=0 python train_with_EATS_v2fix_batch_NTxent_simCLR_120win1_specaug_trtau.py exp_NTxent_simCLR_120_specaug_trtau '100' save-db-query 100k_full # 3:99.39
...
class TrainableTauLayer(tf.keras.layers.Layer):
    
    def __init__(self, init_tau=0.5):
        super(TrainableTauLayer, self).__init__()
        self.tau = tf.Variable(init_tau, trainable=True)
        self.relu = tf.keras.layers.ReLU(max_value=1.)
        
    def call(self, x):
        return x / tf.math.sqrt(self.tau + 1e-5)


# trainable TAU parameter
LR_TAU = 0.1
...
# NTxent-parameters
TAU = 1. # This should be set 1 for trainable tau!
REP_ORG_WEIGHTS = 1.0
...
def train_step(Xa, Xp):
    ...
    feat = m_specaug(feat)
    m_fp.trainable = True
    m_ttl.trainable = True
    with tf.GradientTape() as t1, tf.GradientTape() as t2:
        emb = m_ttl(m_fp(feat))  # (B, E)
        loss, sim_mtx, _ = loss_obj_tr(emb_org=emb[:TR_N_ANCHOR, :],
                                 emb_rep=emb[TR_N_ANCHOR:, :])
    g1 = t1.gradient(loss, m_fp.trainable_variables)
    g2 = t2.gradient(loss, m_ttl.trainable_variables)
    g2[0] = g2[0] * LR_TAU
    opt.apply_gradients(zip(g1, m_fp.trainable_variables))
    opt.apply_gradients(zip(g2, m_ttl.trainable_variables))
    
    # Logging
    tr_loss(loss)
    with tr_summary_writer.as_default():
        tf.summary.scalar('loss', tr_loss.result(), step=opt.iterations)
    return sim_mtx, m_ttl.tau.value()
...
def main():
    if TEST_MODE == 'mini-test-only':
        ...
        start_ep = min(int(opt.iterations) // n_iters, int(EPOCH))
        ...
        for ep in range(start_ep, MAX_EPOCH):
            ...
            while i < len(enq.sequence):
                i += 1
                Xa, Xp = next(enq.get()) 
                sim_mtx, current_tau = train_step(Xa, Xp)
