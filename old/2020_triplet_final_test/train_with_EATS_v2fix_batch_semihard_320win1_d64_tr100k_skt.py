...
CUDA_VISIBLE_DEVICES=0 python train_with_EATS_v2fix_batch_semihard_320win1_d64_tr100k.py exp_v2fix_semihard_320win1_d64_tr100k ''
CUDA_VISIBLE_DEVICES=0 python train_with_EATS_v2fix_batch_semihard_320win1_d64_tr100k.py exp_v2fix_semihard_320win1_d64_tr100k '' mini-test-only
CUDA_VISIBLE_DEVICES=0 python train_with_EATS_v2fix_batch_semihard_320win1_d64_tr100k_ts100kfull.py exp_v2fix_semihard_320win1_d64_tr100k '11' save-query 
CUDA_VISIBLE_DEVICES=0 python eval_faiss.py logs/emb/exp_v2fix_semihard_320win1_d64_tr100k_100kfull/4 2000 ivfpq-rr 

** This model was used for SKT 500k test (new test with 5 controlled offsets)
CUDA_VISIBLE_DEVICES=0 python train_with_EATS_v2fix_batch_semihard_320win1_d64_tr100k_skt.py exp_v2fix_semihard_320win1_d64_tr100k '4' save-query
CUDA_VISIBLE_DEVICES=0 python train_with_EATS_v2fix_batch_semihard_320win1_d64_tr100k_skt.py exp_v2fix_semihard_320win1_d64_tr100k '4' save-db
...
TR_N_ANCHOR = 64 #16  # 8
VAL_BATCH_SZ = 160#320#160
VAL_N_ANCHOR = 32#160#32
TS_BATCH_SZ = 160
TS_N_ANCHOR = 32
#GEN_EMB_BATCH_SZ = 60#240#320
#GEN_EMB_N_ANCHOR = 10#40#160
...
DATA_ROOT_DIR = '/ssd1/music_skt_500k_query/20/'
...
EMB_DIR = 'logs/emb/' + EXP_NAME + '_skt_query'
...
#    fns_event_list=music_fps[:80000],
#    bsz=TR_BATCH_SZ,
...
def generate_emb(n_query=2000000):
    ...
    """
    if n_query == 0: # gen DB
        GEN_EMB_BATCH_SZ = 128#30
        GEN_EMB_N_ANCHOR = 128#30
    else: # gen query
        GEN_EMB_BATCH_SZ = 60
        GEN_EMB_N_ANCHOR = 10
        
    gen_emb_ds = generator_fp.genUnbalSequence(
            music_fps, # NOT reversed list
            GEN_EMB_BATCH_SZ,
            GEN_EMB_N_ANCHOR,
            DUR,
            HOP,
            ...
            ir_mix_parameter=[True, ir_fps],
            test_mode=True)
    print('n_pos_per_anchor = ', gen_emb_ds.n_pos_per_anchor)
    print('---test mode with offsets: {}'.format(gen_emb_ds.test_mode_offset_sec_list))
    ...
    if n_query > 0: # Query generation mode
        n_query = (n_query // n_augs_per_batch) * n_augs_per_batch 
        n_query = min(n_query, len(gen_emb_ds) * GEN_EMB_N_ANCHOR)
        print('---{} queries will be generated---'.format(n_query))
        n_batch_required_for_queries = int(n_query / n_augs_per_batch)
        shape_query = (n_query, n_augs_per_anchor, EMB_SZ)
        ...
        print('---{} iterations required for generating queries---'.format(n_batch_required_for_queries))
        progbar = tf.keras.utils.Progbar(n_batch_required_for_queries)
        
        for i, (Xa, Xp) in enumerate(gen_emb_ds):
            progbar.update(i)
            _, emb_pos = test_step(Xa, Xp, GEN_EMB_N_ANCHOR, GEN_EMB_BATCH_SZ)
        ...
        print('---DB generation mode---')
        progbar = tf.keras.utils.Progbar(len(gen_emb_ds))
        
        for i, (Xa, Xp) in enumerate(gen_emb_ds):
            progbar.update(i)
            emb_anc, _ = test_step(Xa, Xp, GEN_EMB_N_ANCHOR, GEN_EMB_BATCH_SZ)
            db[i*GEN_EMB_N_ANCHOR:(i+1)*GEN_EMB_N_ANCHOR, :] = emb_anc.numpy()
            db.flush();
            
        db.flush(); del(db); # close mmap-files
...
def mini_search_test(n_samples=3000):
    ...
    query = np.empty((0, int((VAL_BATCH_SZ - VAL_N_ANCHOR) / VAL_N_ANCHOR), EMB_SZ))
    n_iter = n_samples // VAL_BATCH_SZ
    
    # Collect mini DB
    for i in range(n_iter):
        Xa, Xp = test_ds.__getitem__(i)
        emb_anc, emb_pos = test_step(Xa, Xp)
        emb_pos = tf.reshape(emb_pos, (VAL_N_ANCHOR, -1, EMB_SZ)) 
...
def main():
    ...
    elif TEST_MODE == 'save-query':
        generate_emb(n_query=2396000)
    else:
