...
OPTIONS:
    ...
    - If <opt1>, <opt2> and <opt3:TEST_MODE> are given:
        ...
        - If <opt3:TEST_MODE> == "save-db-only", it loads pre-trained model and only saves embeddings for DB as a numpy file.
        - If <opt3:TEST_MODE> == "save-db", it loads pre-trained model and saves embeddings for DB and Query as numpy files.
LOG-DIRECTORIES:
...
from model.nnfp_l2norm_v2 import FingerPrinter
...
#%%
#EXP_NAME = 'exp_v2_semihard_240'
#EPOCH = ''
#TEST_MODE = 'mini-test-only'
...
def generate_emb(n_query=100000):
    ...
    shape_db = (len(test_ds) * TS_BATCH_SZ, EMB_SZ)
    shape_query = (len(test_ds) * TS_BATCH_SZ, n_augs_per_anchor, EMB_SZ)
    
    # Create memmap, and save shapes
    db = np.memmap(EMB_DIR + '/db.mm', dtype='float32', mode='w+', shape=shape_db)
    np.save(EMB_DIR + '/db_shape.npy', shape_db)
    if (n_query > 0):
        query = np.memmap(EMB_DIR + '/query.mm', dtype='float32', mode='w+', shape=shape_query)
        np.save(EMB_DIR + '/query_shape.npy', shape_query)
    ...
    for i, (Xa, Xp) in enumerate(test_ds):
        ...
        if (i < n_batch_required_for_queries) & (n_query != 0):
            ...
            query[i*n_augs_per_batch:(i+1)*n_augs_per_batch, :, : ] = emb_pos.numpy()
    
    tf.print('------Succesfully saved embeddings to {}-----'.format(EMB_DIR))
