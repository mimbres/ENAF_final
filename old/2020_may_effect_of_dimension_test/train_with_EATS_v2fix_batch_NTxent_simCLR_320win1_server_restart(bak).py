...
CUDA_VISIBLE_DEVICES=1 python train_with_EATS_v2fix_batch_NTxent_simCLR_320win1_server_lr1e-4.py exp_NTxent_simCLR_320_ln_drop_lamb_sch1e04
...
if LR_SCHEDULE == 'cos':
    lr_schedule = tf.keras.experimental.LinearCosineDecay(initial_learning_rate=LR,
        decay_steps=len(get_train_ds()) * MAX_EPOCH, num_periods=0.5, alpha=0, beta=2e-07)
    ...
    lr_schedule = tf.keras.experimental.CosineDecayRestarts(initial_learning_rate=LR,
        first_decay_steps=int(len(get_train_ds())*0.1), alpha=2e-07)
    opt = OPTIMIZER(learning_rate=lr_schedule)
else:
