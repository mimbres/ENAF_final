...
def _pairwise_distances(embeddings, squared=False):
    ...
    Args:
        embeddings: tensor of shape (B, Q)
        squared: Boolean. If true, output is the pairwise squared euclidean distance matrix.
                 If false, output is the pairwise euclidean distance matrix.
    Returns:
        pairwise_distances: tensor of shape (B, B)
    """
    dot_product = tf.matmul(embeddings, tf.transpose(embeddings)) # (B,B)

    # Get squared L2 norm for each embedding. We can just take the diagonal of `dot_product`.
    square_norm = tf.linalg.diag_part(dot_product) # (B,)

    # Pairwise distance matrix: # ||a - b||^2 = ||a||^2  - 2 <a, b> + ||b||^2
    dists = tf.expand_dims(square_norm, 1) - 2.0 * dot_product + tf.expand_dims(square_norm, 0) # (B,B)
...
def batch_all_triplet_loss(labels, embeddings, margin, squared=False):
    ...
    Args:
        labels: labels of the batch, of size (B,)
        embeddings: tensor of shape (B,Q) = (batch_size, embed_dim)
...
def batch_hard_triplet_loss(labels, embeddings, margin, squared=False):
    ...
    # We put 0 to any element where (a, p) is not valid (valid if a != p and label(a) == label(p))
    ...
    return triplet_loss, 0 # Ignore fraction output 0 here...