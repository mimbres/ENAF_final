...
#                                [(1,1), (2,1)], [(1,2), (2,1)]],
        ...
        if self.q_adapt_layer:# UPDATE for q-adapt
            # self.enc_key = DivEncLayer(q=emb_sz, unit_dim=fc_unit_dim, norm=norm)
            # self.enc_que = DivEncLayer(q=emb_sz, unit_dim=fc_unit_dim, norm=norm)
            self.enc_key = tf.keras.Sequential(
                [tf.keras.layers.Dense(256, activation='elu'),
                tf.keras.layers.LayerNormalization(axis=-1),
                tf.keras.layers.Dense(emb_sz)])
            self.enc_que = tf.keras.Sequential(
                [tf.keras.layers.Dense(256, activation='elu'),
                tf.keras.layers.LayerNormalization(axis=-1),
                tf.keras.layers.Dense(emb_sz)])
        else:
    ...
    def call(self, inputs):
        ...
        if self.q_adapt_layer:
            x_key = self.enc_key(x)
            x_que = self.enc_que(x)
            if self.use_L2layer:# UPDATE for q-adapt
                return tf.math.l2_normalize(x_key, axis=1), tf.math.l2_normalize(x_que, axis=1)
            else:
                return x_key, x_que
        else:
...
def test():
    ...
    #%timeit -n 10 fprinter(_input) # 18.1ms
    # 14M
    return fprint
