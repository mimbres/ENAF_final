...
class Conv2dLayer(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        self.conv2d_3x3 = tf.keras.layers.Conv2D(hidden_ch,
                                                 kernel_size=(3, 3),
                                                 strides=strides,
                                                 padding='SAME',
                                                 dilation_rate=(1, 1),
        ...
        if norm == 'layer_norm1d':
            self.BN_3x3 = tf.keras.layers.LayerNormalization(axis=-1)
        elif norm == 'layer_norm2d':
            self.BN_3x3 = tf.keras.layers.LayerNormalization(axis=(1, 2, 3))
        else:
            self.BN_3x3 = tf.keras.layers.BatchNormalization(axis=-1) # Fix axis: 2020 Apr20
...
class FingerPrinter(tf.keras.Model):
    ...
    def __init__(self,
        ...
        for i in range(self.n_clayers):
            self.front_conv.add(Conv2dLayer(hidden_ch=front_hidden_ch[i],
...
<2d conv>
Total params: 19,661,184
Trainable params: 19,661,184
