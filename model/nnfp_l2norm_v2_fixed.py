...
class FingerPrinter(tf.keras.Model):
    ...
    def __init__(self,
                 ...
                 norm='batch_norm',
                 use_L2layer=True,
                 use_effnet_ver=False):
        super(FingerPrinter, self).__init__()
        self.norm = norm
        self.use_L2layer = use_L2layer
        self.use_effnet_ver = use_effnet_ver
        self.n_clayers = len(front_strides)
        
        if (use_effnet_ver > 0):
            self.front_conv = tf.keras.Sequential(name=f'EfficentNet{use_effnet_ver}')
            self.front_conv.add(EFFNETS[self.use_effnet_ver](include_top=False, weights=None,
                input_tensor=None, input_shape=input_shape, pooling='avg'))
        else:
