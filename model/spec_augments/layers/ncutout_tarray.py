...
"""SpecNCutout.
: Batch-wise N-Cut-out for Audio Spectogram with GPUs.
...
Created on Fri Apr 24 22:34:51 2020
...
class SpecNCutout(tf.keras.layers.Layer):
    ...
    def generate_single_mask(self, h_start=int(), h_end=int(), w_start=int(),
        ...
        # mask = tf.logical_or(m_h, m_w) # Freq-time mask mode 
        return mask # (h, w)
...
def test():
    ...
    # 1.88 ms ± 305 µs per loop (mean ± std. dev. of 7 runs, 20 loops each)
    # 38.4 ms ± 1 ms per loop (mean ± std. dev. of 7 runs, 10 loops each)
    # 82 ms ± 1.91 ms per loop (mean ± std. dev. of 7 runs, 10 loops each)
    # 359 ms ± 26.9 ms per loop (mean ± std. dev. of 7 runs, 10 loops each)
