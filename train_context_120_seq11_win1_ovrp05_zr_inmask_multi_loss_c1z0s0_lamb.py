...
CUDA_VISIBLE_DEVICES=0 python train_context_120_seq11_win1_ovrp05_zr_inmask_multi_loss_c1z0s0_lamb.py exp_context_120_seq11_win1_ovrp05_zr_inmask_multi_loss_c1z0s0_lamb
CUDA_VISIBLE_DEVICES=0 python train_context_120_seq11_win1_ovrp05_zr_inmask_multi_loss_c1z0s0_lamb.py exp_context_120_seq11_win1_ovrp05_zr_inmask_multi_loss_c1z0s0_lamb '1' mini-test-only
CUDA_VISIBLE_DEVICES=6 python train_context_120_seq11_win1_ovrp05_zr_inmask_multi_loss_c1z0s0_lamb.py exp_context_120_seq11_win1_ovrp05_zr_inmask_multi_loss_c1z0s0_lamb '100' save-db-query 100k_full # 3:99.39
CUDA_VISIBLE_DEVICES=4 OMP_NUM_THREADS=12 python eval_faiss.py gpu logs/emb/exp_context_120_seq11_win1_ovrp05_zr_inmask_multi_loss_c1z0s0_lamb/100/100k_full 2000 ivfpq-rr 20 mirex

ep47
---------ArgMin g(f(x)) mini-TEST----------
scope:   [1, 3, 5, 9, 11, 19]
T1acc: 69.10  89.29  94.71  97.21  97.97  99.16
mrank: 9.41  0.83  0.17  0.04  0.02  0.01
-----------ArgMin f(x) mini-TEST-----------
scope:   [1, 3, 5, 9, 11, 19]
T1acc: 68.19  87.55  93.18  95.81  96.92  99.02
mrank: 5.44  0.76  0.24  0.06  0.04  0.01
---------ArgMin l2(f(x)) mini-TEST---------
scope:   [1, 3, 5, 9, 11, 19]
T1acc: 68.40  87.83  93.25  95.60  96.85  99.02
mrank: 5.19  0.75  0.25  0.06  0.04  0.01

MIREX
matched_exact=[0.71 2.72 3.53 6.05 6.86 8.68]%, scope=[1, 3, 5, 9, 11, 19]
matched_near=[ 3.73  9.49 14.33 21.09 23.71 32.19]%, scope=[1, 3, 5, 9, 11, 19]
matched_song=[ 4.34  9.89 15.34 21.7  24.52 32.59]%, scope=[1, 3, 5, 9, 11, 19]

10k
matched_exact=[38.22 79.01 89.65 96.33 97.29 99.  ]%, scope=[1, 3, 5, 9, 11, 19]
matched_near=[39.58 79.41 89.8  96.33 97.29 99.  ]%, scope=[1, 3, 5, 9, 11, 19]
matched_song=[46.01 83.17 91.66 97.34 97.89 99.05]%, scope=[1, 3, 5, 9, 11, 19]

100k_full
matched_exact=[11.5  35.81 48.97 62.48 66.1  75.74]%, scope=[1, 3, 5, 9, 11, 19]
matched_near=[11.9  35.86 49.02 62.48 66.1  75.89]%, scope=[1, 3, 5, 9, 11, 19]
matched_song=[16.12 41.39 55.25 66.75 70.07 79.16]%, scope=[1, 3, 5, 9, 11, 19]
...
WEIGHT_MULTI_LOSS = [1., 0.] # [context_loss, simclr_loss]
...
H_MAX_POS_ENC = 11 # int(SEQ_SAMPLING_DUR / SEG_HOP) - 1
...
# Hyper-parameters
SEQ_SCOPE_SEC = 6.144 # sec
SEQ_SAMPLING_DUR = 6.144#10.
...
def get_test_ds(no_aug=False):
    ...
    if no_aug:
        ...
        ds = generator_fp.genUnbalSequence(
            ...
            bg_mix_parameter=[True, aug_ts_fps, (10, 10)],
            ir_mix_parameter=[True, ir_fps],
            speech_mix_parameter=[False]#[True, speech_tr_fps, (0,10)]
            )
    return ds
