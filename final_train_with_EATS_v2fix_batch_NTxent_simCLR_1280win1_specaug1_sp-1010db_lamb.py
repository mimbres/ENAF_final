...
CUDA_VISIBLE_DEVICES=1 OMP_NUM_THREADS=12 python final_train_with_EATS_v2fix_batch_NTxent_simCLR_1280win1_specaug1_sp-1010db_lamb.py final_exp_NTxent_simCLR_1280win1_specaug1_sp-1010db_lamb
CUDA_VISIBLE_DEVICES=1 OMP_NUM_THREADS=12 python final_train_with_EATS_v2fix_batch_NTxent_simCLR_1280win1_specaug1_sp-1010db_lamb.py final_exp_NTxent_simCLR_1280win1_specaug1_sp-1010db_lamb '1' mini-test-only
CUDA_VISIBLE_DEVICES=0 OMP_NUM_THREADS=12 python final_train_with_EATS_v2fix_batch_NTxent_simCLR_1280win1_specaug1_sp-1010db_lamb.py final_exp_NTxent_simCLR_1280win1_specaug1_sp-1010db_lamb '63' save-db-query-all 
CUDA_VISIBLE_DEVICES=0 OMP_NUM_THREADS=12 python eval_faiss.py gpu logs/emb/final_exp_NTxent_simCLR_1280win1_specaug1_sp-1010db_lamb/100/10k 2000 ivfpq-rr 20 mirex

ep63
MIREX
matched_exact=[10.08 14.8  16.78 18.38 18.85 21.58]%, scope=[1, 3, 5, 9, 11, 19]
matched_near=[37.42 52.5  62.11 73.33 75.87 82.75]%, scope=[1, 3, 5, 9, 11, 19]
matched_song=[37.42 52.5  62.11 73.33 75.87 82.75]%, scope=[1, 3, 5, 9, 11, 19]

task=10k, use_gpu=True, mode=ivfpq-rr, n_probe20, n_test2000
elapse time per 10 iterations: 0.2499 sec.
1999/2000
matched_exact=[86.1  96.45 97.85 98.85 99.25 99.7 ]%, scope=[1, 3, 5, 9, 11, 19]
matched_near=[89.5  96.7  98.05 98.9  99.3  99.7 ]%, scope=[1, 3, 5, 9, 11, 19]
matched_song=[89.5  96.7  98.05 98.9  99.3  99.7 ]%, scope=[1, 3, 5, 9, 11, 19]

task=100k_full, use_gpu=True, mode=ivfpq-rr, n_probe20, n_test2000
elapse time per 10 iterations: 1.0813 sec.
1999/2000
matched_exact=[64.5  84.45 89.35 93.25 93.85 96.05]%, scope=[1, 3, 5, 9, 11, 19]
matched_near=[66.7  84.95 89.65 93.3  93.95 96.05]%, scope=[1, 3, 5, 9, 11, 19]
matched_song=[66.7  84.95 89.65 93.3  93.95 96.05]%, scope=[1, 3, 5, 9, 11, 19]

ep100----
mirex
matched_exact=[10.08 14.8  16.59 18.57 18.66 20.83]%, scope=[1, 3, 5, 9, 11, 19]
matched_near=[36.48 52.21 61.17 73.33 75.87 82.  ]%, scope=[1, 3, 5, 9, 11, 19]
matched_song=[36.48 52.21 61.17 73.33 75.87 82.  ]%, scope=[1, 3, 5, 9, 11, 19]

task=10k, use_gpu=True, mode=ivfpq-rr, n_probe20, n_test2000
elapse time per 10 iterations: 0.2224 sec.
1999/2000
matched_exact=[85.6  96.35 97.8  98.85 99.15 99.7 ]%, scope=[1, 3, 5, 9, 11, 19]
matched_near=[89.2  96.75 98.05 98.9  99.25 99.7 ]%, scope=[1, 3, 5, 9, 11, 19]
matched_song=[89.2  96.75 98.05 98.9  99.25 99.7 ]%, scope=[1, 3, 5, 9, 11, 19]

task=100k_full, use_gpu=True, mode=ivfpq-rr, n_probe20, n_test2000
elapse time per 10 iterations: 0.9798 sec.
1999/2000
matched_exact=[64.   83.85 89.2  93.   93.5  95.9 ]%, scope=[1, 3, 5, 9, 11, 19]
matched_near=[66.2  84.3  89.35 93.15 93.7  96.05]%, scope=[1, 3, 5, 9, 11, 19]
matched_song=[66.2  84.3  89.35 93.15 93.7  96.05]%, scope=[1, 3, 5, 9, 11, 19]
...
from EATS import generator_fp_dm as generator_fp #generator_fp
...
TR_BATCH_SZ = 1280#240 #320 #80  #40
TR_N_ANCHOR = 640 #120 #64 #16  # 8
VAL_BATCH_SZ = 120
VAL_N_ANCHOR = 60
...
def get_train_ds():
    ds = generator_fp.genUnbalSequence(
        ...
        speech_mix_parameter=[True, speech_tr_fps, (-10,10)])
    return ds
...
def get_val_ds():
    ds = generator_fp.genUnbalSequence(
        ...
        ir_mix_parameter=[True, ir_fps],
        speech_mix_parameter=[True, speech_tr_fps, (0,10)])
    return ds

def get_test_ds(no_aug=False, gen_emb_datasel=GEN_EMB_DATASEL):
    ...
    if no_aug:
        ds = generator_fp.genUnbalSequence(
            ...
            TS_BATCH_SZ, TS_BATCH_SZ, DUR, HOP, FS, shuffle=False, random_offset=False) 
    else:
        ds = generator_fp.genUnbalSequence(
            list(reversed(test_fps)), # reversed list for later on evaluation!!
            TS_BATCH_SZ,
...
if LR_SCHEDULE == 'cos':
    ...
    opt = OPTIMIZER(learning_rate=LR, clipnorm=GRAD_CLIP_NORM)
...
#%% test functions...
def generate_emb(n_query=300000, gen_emb_datasel=GEN_EMB_DATASEL):
    """
    Arguemnts:
    ...
    """
    test_db_ds = get_test_ds(no_aug=True, gen_emb_datasel=gen_emb_datasel)
    test_query_ds = get_test_ds(no_aug=False, gen_emb_datasel=gen_emb_datasel)
...
def main():
    ...
    elif TEST_MODE == 'save-db-query-all':
        generate_emb_mirex()
        emb_dir = generate_emb(n_query=300000, gen_emb_datasel='10k')
        eval(use_gpu=True, emb_dir=emb_dir, n_sample_test=2000, mode='ivfpq-rr',
             n_probe = 20, display_interval=10, test_mirex_query=True) # 10k mirex test
        eval(use_gpu=True, emb_dir=emb_dir, n_sample_test=2000, mode='ivfpq-rr',
             n_probe = 20, display_interval=10, test_mirex_query=False) # 10k test
        emb_dir = generate_emb(n_query=300000, gen_emb_datasel='100k_full')
        eval(use_gpu=True, emb_dir=emb_dir, n_sample_test=2000, mode='ivfpq-rr',
             n_probe = 20, display_interval=10, test_mirex_query=False) # 100k_full test
    else:
