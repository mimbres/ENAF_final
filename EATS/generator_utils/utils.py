...
def bg_mix_batch(event_batch, bg_batch, fs, snr_range=(6,24), unit='db', mode='energy'):
    ...
    event_amp_ratio_batch = log_scale_random_number_batch(bsz=len(event_batch), amp_range=(0.1,1))
    ...
    for i in range(len(event_batch)):
        ...
        if event_max == 0 or bg_max == 0:
            ...
            X_bg_mix[i] = core.max_normalise(X_bg_mix[i])

        else:
            mix_audio, snr_db, _, _ = mix.background_mix(x=event_batch[i],
                                                         x_bg=bg_batch[i],
                                                         fs=fs,
                                                         snr=snrs[i],#snr_range=snr_range, <-- was not actually random
                                                         unit=unit,
                                                         mode=mode)
            #mix_audio = 1*bg_batch[i]/bg_max+0*event_batch[i]/event_max
            
            X_bg_mix[i] = mix_audio
            #print(snr_db)
...
#                if (offset_max < 0):
#                    print('\n offset_min={}, offset_max={}, seg_idx={}, residual={}, n_frames_in_hop={}',
#                          offset_min, offset_max, seg_idx, residual_frames, n_frames_in_hop)
...
def load_audio(filename=str(),
    ...
    # WAV: Open, Get file info --> Gauaging --> Load  --> padding, normalize

    # Gauging: start_frame_idx, end_frame_idx, pad_left, pad_right
...
def load_audio_multi_start(filename=str(),
    ...
    for seg_start_sec in seg_start_sec_list:
        x = load_audio(filename=filename, seg_start_sec=seg_start_sec, seg_length_sec=seg_length_sec, fs=8000)
        x = x.reshape((1,-1))
        if out is None:
            out = x
        else:
...
#%%
def calculate_offset_padoffset(offset_max=0, pad_mode='random', random_seed=0, fs=22050):

    seg_pad_offset_sec = 0.0

    if offset_max > 0 : # don't do padding if win_length > audio_length
        np.random.seed(random_seed) # This is needed for random seed!
        offset_frame = np.random.randint(offset_max)
        offset_sec = offset_frame/fs

    elif offset_max == 0 : # if win_length == audio_length
        offset_sec=0.0

    else : # do padding if win_length < audio_length
        offset_max=np.abs(offset_max)
        if pad_mode == 'random':
            np.random.seed(random_seed) # This is needed for random seed!
            seg_pad_offset_sec = np.random.randint(offset_max)/fs
        elif pad_mode == 'right':
            offset_sec = 0.0

    return offset_sec, seg_pad_offset_sec
...
def npy_to_wav(root_dir=str(), source_fs=int(), target_fs=int()):
    ...
    for fname in fns:
        ...
        wavio.write(fname[:-4] + '.wav', audio, target_fs, sampwidth=2) # sampwidth=2 for 16bit
